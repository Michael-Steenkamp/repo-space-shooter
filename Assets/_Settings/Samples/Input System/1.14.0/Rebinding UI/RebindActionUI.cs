...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            string originalPath = targetBinding.path; // The default/original path
            string currentOverridePath = targetBinding.overridePath;

            // Remove override on this binding
            action.RemoveBindingOverride(bindingIndex);

            // Check for conflicts in the same action map
            foreach (InputAction otherAction in action.actionMap.actions)
            {
                for (int i = 0; i < otherAction.bindings.Count; i++)
                {
                    if (otherAction == action && i == bindingIndex) continue;

                    InputBinding otherBinding = otherAction.bindings[i];

                    // Conflict: another binding currently uses the original path we're trying to restore
                    if (otherBinding.effectivePath == originalPath)
                    {

                        // If other binding has an override, preserve it before swap
                        string otherOriginalPath = otherBinding.path;

                        // Swap the overrides
                        action.ApplyBindingOverride(bindingIndex, originalPath);
                        otherAction.ApplyBindingOverride(i, currentOverridePath);
            ...
            // No conflict — just restore normally
            action.ApplyBindingOverride(bindingIndex, originalPath);
        }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // disable the action before use to prevent errros
            action.Disable();
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnCancel(
                    operation =>
                    {
                        action.Enable(); // Re-enable the action after rebinding.
        ...
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // 1. Check other bindings in the same action
            ...
            // 2. Check bindings in other actions in the same action map
            ...
            // 3. Optional: check composite parts
            if (allCompositeParts)
            {
                for (int i = 0; i < action.bindings.Count; i++)
                {
                    ...
                    if (action.bindings[i].isPartOfComposite &&
                        action.bindings[i].effectivePath == newBinding.effectivePath)
                    {
                        return true;
                    }
                }
        ...
        [Tooltip("Optional text that will be used to override the action label. Only used if "
            + "m_OverrideActionLabel is true.")]
        [SerializeField]
        private string m_ActionLabelOverrideString = "Action Name";
        ...
        private void UpdateActionLabel()
        {
            if (m_ActionLabel != null)
            {
                ...
                else
                {
                    ...
                    m_ActionLabelOverrideString = string.Empty;
                }
